...
namespace init {
    static bool useVerticalSyn = true; //垂直同步
    // 设置版本
    ...
    GLFWwindow* Init(const std::string& windowName, const unsigned int& width, const unsigned int& height, const unsigned int& versionNum_1, const unsigned int& versionNum_2, const bool& isCore)
    {
        ...
        SetWindowIcon(window, ICON_PATH); // 设置图标
        glfwMakeContextCurrent(window); /* Make the window's context current */

        if(useVerticalSyn) GLCall(glfwSwapInterval(1)); // 设置垂直同步

        GLCall(glEnable(GL_CULL_FACE)); // 开启面剔除  没生效???
